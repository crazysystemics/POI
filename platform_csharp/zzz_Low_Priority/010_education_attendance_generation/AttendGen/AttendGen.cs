...
namespace AttendGen
{
    class AttendGen
    {
        ...
        public string Calibrate(string ins)
        {
            ...
            int condonation=0;
            if (!int.TryParse(sarr[poscondon], out condonation))
            {
                condonation = 0;
            }
            ...
            //
            int requiredAttendance85 = Convert.ToInt32((double)maxAttendance * .86);
            ...
            if (presents + condonation < requiredAttendance85)
            {
                corrected = true;
                int gapAttendance = requiredAttendance85 - presents;
                int absgapdiff = absents - gapAttendance;

                //TODO:PARK IT BASED ON COLUMN NAME

                if ((double)totalAttendance / (double)maxAttendance > .86)
                    return ins;

                if (totalAttendance < 40)
                {
                ...
                for (presents = 0, index = POS_ATTEND_MIN; index <= POS_ATTEND_MAX; index++)
                {
                    if (sarr[index] != "A")
                    {
                        presents++;
            ...
            //condonation entry is done manually.
            //since presents count includes condonation, it should not
            //be added specifically.
            int total = presents;
            if (!corrected) total += condonation;
