...
namespace uassociit
{
    ...
    {
        public void WriteLine(string s, ConsoleColor bg, ConsoleColor fg, bool reset = true)
        {
                        ...
                        ConsoleColor[] bg, ConsoleColor[] fg, string delim = " ")
        {
            ...
            Console.BackgroundColor = tbg;
            Console.ForegroundColor = tfg;
            Console.WriteLine(title);// writing title
            Console.ResetColor();
        ...
        //PrintMatrix V3
        public void PrintMatrix(string title, string[,] Matrix, int rows, int cols,
                                string[] symbols,
                                ConsoleColor tbg, ConsoleColor tfg,
                                ConsoleColor defbg, ConsoleColor deffg,
                                ConsoleColor[] symbg, ConsoleColor[] symfg, string delim = " ")
        {
            for (int i = 0; i < rows; i++)
            {
                for (int j = 0; j < rows; j++)
                {
                    int key = symbols.ToList().FindIndex(x => x == Matrix[i, j].First().ToString());

                    if (key >= 0)
                    {
                        Write(Matrix[i, j], symbg[key], symfg[key], false);
                    }
                    else
                    {
                        Write(Matrix[i, j], defbg, deffg, false);
                    }
                }

                WriteLine("", ConsoleColor.Black, ConsoleColor.White, false);
            }
