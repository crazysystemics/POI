public class GCIRadar : Radar
{
    ...
    public GCIRadar(Pulse initPulse, Position position, int pulseRepetitionInterval, int txTick, int radius, int effectiveRadiatedPower, int id) : base(initPulse, position, pulseRepetitionInterval, txTick, radius, effectiveRadiatedPower, id)
    {
        int pulseWidth = (int)((Globals.randomNumberGenerator.Next(10, 15) / 100) * pulseRepetitionInterval);
        this.txPulse = new Pulse(pulseWidth, 10, Globals.randomNumberGenerator.Next(2700, 3100), 0, 0);
        this.activePulse = txPulse;
        this.position = position;
        this.id = id;
        this.radius = radius;
        this.txTick = txTick;
        this.pulseRepetitionInterval = Globals.randomNumberGenerator.Next(1500, 2500);
